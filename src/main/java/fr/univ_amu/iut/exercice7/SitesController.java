...
public class SitesController {
  ...
  private void initialize() {
    ...
    colNumero.setCellValueFactory(
        c -> new SimpleStringProperty(String.valueOf(c.getValue().numeroCarre())));
    colNom.setCellValueFactory(
        c -> new SimpleStringProperty(String.valueOf(c.getValue().nomConvivial())));
    colProtocole.setCellValueFactory(
        c -> new SimpleStringProperty(String.valueOf(c.getValue().protocole())));

    tableSites.setItems(viewModel.sitesProperty());

    labelResume.textProperty().bind(viewModel.resumeProperty());

    choiceProtocole.getItems().add("PointFixeStandard");
    choiceProtocole.getItems().add("PointFixeRecherche");
